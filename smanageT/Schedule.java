...
//					
						...
						if(vector2.get(j).get(6).equals("회사")) {
							System.out.println(vector2.get(j).get(6));
...
//			}
		...
		System.out.println("마우스 이벤트값"+obj);
		...
		for(int k=0; k<vector2.size();k++) {
			...
			if(temp.getText().startsWith((String)vector2.get(k).get(2)+".")) {
				System.out.println("인덱스 시작값############### ");
				System.out.println("스타트위즈 비교값"+(String)vector2.get(k).get(2)+".");
				...
				v.bt4.addMouseListener(new MouseListener() {
					...
					public void mouseClicked(MouseEvent e) {
						try {
							...
							System.out.println("실형결과::"+a);
							System.out.println("INDEX"+index);
							v.newntc.setVisible(false);
							System.out.println("다이얼로그 수정 완료");
